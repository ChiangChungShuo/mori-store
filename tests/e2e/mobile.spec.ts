...
test('opens and closes the storefront navigation at 375 px', async ({ page }) => {
  ...
  const upperCategory = categories.locator('details').filter({ hasText: '上衣' }).first()
  await expect(upperCategory).not.toHaveAttribute('open', '')
  await upperCategory.locator('summary').click()
  await expect(upperCategory).toHaveAttribute('open', '')
  await expect(upperCategory.getByRole('link', { name: '全部上衣' })).toBeVisible()
  await expect(upperCategory.getByRole('link', { name: 'Mori flora 漫花系列' })).toBeVisible()
